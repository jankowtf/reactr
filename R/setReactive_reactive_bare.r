...
#' @template threedot
#' @example inst/examples/setReactive_reactive_bare.r
#' @seealso \code{
...
#' @template references
#' @export 
setReactive_reactive_bare <- function(
    id,
    value = NULL,
    where = .GlobalEnv,
    watch = character(),
...
#   binding_type <- as.numeric(match.arg(as.character(binding_type), c("1", "2")))
  ...
  if (inherits(binding, "reactive")) {
    .binding <- substitute(function(){BINDING}, list(BINDING = binding))
    if (!mutual) {
